<div class="functeBtns st2">
    ...
    @if ($value->has('referencePurchased')) {{--ー分岐見直す--}}
        @if ($value->purchased_reference_type === 'App\Models\PurchasedProduct')
            <a href="{{route('chatroom.show.purchased_product', $value->id)}}" class="blue_o">チケット詳細</a>
        @elseif ($value->purchased_reference_type === 'App\Models\PurchasedJobRequest')
            <a href="{{route('chatroom.show.purchased_job_request', $value->id)}}" class="blue_o">チケット詳細</a>
        @endif
