                            ...
                            <tbody>
                                @foreach($job_requests as $job_request)
                                    <tr>
                                        ...
                                        <td class="px-2">{{ App\Models\JobRequest::IS_ONLINE[$job_request->is_online] ?? "" }}</td>
                                        <td class="px-2">{{ $job_request->prefecture?->name }}</td>
                                        @if ($job_request->is_draft == 1)
                                            <td class="px-2">{{ App\Models\JobRequest::DRAFT_STATUS[$job_request->is_draft] }}</td>
                                        @else
                                            <td class="px-2">{{ App\Models\JobRequest::SALES_STATUS[$job_request->status] }}</td>
                                        @endif
